...
runastrusteduser()
{
	...
	setreuid(0, (uid_t)-1);		/* set real uid to root */
	setreuid((uid_t)-1, trusteduid); /* set euid to trusted */
...
runasrootuser()
{
	...
	setreuid((uid_t)-1, 0);		/* make us effectively root */
	setreuid(origruid, (uid_t)-1);	/* reset to old real uid */
}
