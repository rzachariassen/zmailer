...
sp_init()
{
	...
	q = (struct sptree *) emalloc(sizeof(*q));
...
struct spblk *
_sp_deq(np)
	struct spblk **np;		/* pointer to a node pointer */
{
	register struct spblk
			*deq,		/* one to return */
