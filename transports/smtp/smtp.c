	...
	char *argv[];
{
	...
	SS.remotehost[0] = '\0';
	while (1) {
	  c = getopt(argc, argv, "c:deh:l:p:rsvxDEF:L:HPT:VWZ:78");
	  if (c == EOF)
	    break;
	  switch (c) {
	...
	while (!getout) {
	  ...
	  if (!SS.smtpfp) {
		...
		smtpstatus = EX_OK; /* We were forcibly aborted .. */
	      }
	      procabortset = 0;
	    }
		...
		diagnostic(rp, smtpstatus, 60, "%s", SS.remotemsg);
	      }
	    }
	  } else
...
if (SS->verboselog)
  fprintf(SS->verboselog,"getaddrinfo('%s','smtp') -> r=%d, ai=%p\n",buf,rc,ai);
	  ...
	  SS->mxh[0].host = NULL;	/* use the hostent we got */
#ifdef	BIND
	  /*
