     ...
     LDAPMAP *lmap;
{
    ...
    if( ( lmap->ldaphost != NULL || lmap->ldapport ) && ( lmap->uri == NULL ) ) {
    ...
    if( ldap_set_option( lmap->ld, LDAP_OPT_PROTOCOL_VERSION, &lmap->protocol )
        != LDAP_OPT_SUCCESS ) {
            fprintf( stderr, "Could not set LDAP_OPT_PROTOCOL_VERSION %d\n", &lmap->protocol);
    }
#ifdef HAVE_OPENSSL 
    if (lmap->use_tls && ( ldap_start_tls_s( lmap->ld, NULL, NULL ) != LDAP_SUCCESS )) {
    ...
    if (lmap->sasl_secprops != NULL) {
        rc = ldap_set_option( lmap->ld, LDAP_OPT_X_SASL_SECPROPS, (void *) lmap->sasl_secprops);
    ...
    if (lmap->authmethod == LDAP_AUTH_SASL) {;
        if ( ldap_sasl_bind_s( lmap->ld, lmap->binddn, lmap->sasl_mech, &passwd, NULL, NULL, NULL ) 
            != LDAP_SUCCESS ) {
